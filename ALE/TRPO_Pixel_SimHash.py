...
class TRPO(object):
    def __init__(self, env, env_eval, gamma=0.995, lr_c=1e-3, k=256, beta=0.01, save_folder=None):
        self.env = env
    ...
    def update_agent(self, delta=0.01, backtrack_ratio=0.8, max_backtracks=15, damping=1e-1):
        ...
        shs = torch.dot(stepdir, Fvp(stepdir))
        ...
        for i in range(num_epoch):
            ...
            while True:  # episodes loop
                ...
                while True:
                    ...
                    sample.append((input_stack, action, reward, next_state.flatten()))
                    ...
                    if epoch_t >= update_step:
                        with torch.no_grad():
                            input_stack, action, reward, next_state_flat = sample.pop()
                            next_stack = torch.from_numpy(np.array(prev_states[-4:])).float().unsqueeze(0)
                            reward = reward + self.gamma * self.critic(next_stack)
                            sample.append((input_stack, action, reward.item(), next_state_flat))
                        break

                states, actions, rewards, next_states = zip(*sample)
                keys = self.simhash.get_keys(next_states)
